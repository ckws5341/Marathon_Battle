...
#include "Support_Force_General.h"
#include "Misc/Paths.h"				
#include "EngineUtils.h"

// Sets default values
ASupport_Force_General::ASupport_Force_General()
{
...
void ASupport_Force_General::BeginPlay()
{
	Super::BeginPlay();
	...
	FString walking_filename = FPaths::ProjectContentDir() + "bvh/general_bvh/walking.bvh";
	FString raise_hand_filename = FPaths::ProjectContentDir() + "bvh/general_bvh/raise_hand.bvh";
	FString waiting_filename = FPaths::ProjectContentDir() + "bvh/general_bvh/standing.bvh";

	ml::Motion	waiting;
	ml::Motion	walking;
	ml::Motion	raise_hand;

	ml::LoadBVH_UE4(T_pose_filename, waiting_filename, waiting);
	ml::LoadBVH_UE4(T_pose_filename, walking_filename, walking);
	ml::LoadBVH_UE4(T_pose_filename, raise_hand_filename, raise_hand);

	motion_ = walking;
	motion_.Stitch_UE4(walking);
	motion_.Stitch_UE4(walking);
	motion_.Stitch_UE4(raise_hand);
...
bool ASupport_Force_General::InitPoseableCharacter()
{
	...
	{
		if (components[i]->GetName() == "support_gen")
...
// Called every frame
void ASupport_Force_General::Tick(float DeltaTime)
{
